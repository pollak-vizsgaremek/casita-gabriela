...
const hideSideBar = () => {
  ...
  return (
    <div className='z-10 overflow-visible'>
        <nav className=' bg-[#D9F5E3]  w-dvw h-[10dvh] flex items-center px-4 shadow-md  z-40 border-b-2 border-gray-200 fixed top-0'>
            ...
            <div className='ml-auto flex gap-6 align-center '>
                {routes.slice(1).map((r, index) => (
                    <Link to={r.route} 
                    className={index === 2 ? 'bg-[#6FD98C] hidden sm:inline text-white px-3 py-1 rounded-xl hover:-translate-y-px hover:bg-[#5FCB80] transition-all duration-200 mr-5 w-25 text-center font-bold h-full'
                       : 
                    'text-[#1F1F1F] px-3 py-1 hidden sm:inline font-bold h-full hover:text-[#515151] hover:-translate-y-px transition-all duration-200'} key={r.route}>
                        {r.name}
                    </Link>
                ))}
        ...
        <nav  ref={navRef} className=' bg-[#D9F5E3] sidebar w-[200px] h-dvh fixed right-0 top-0 items-center px-4 shadow-md  z-50 border-b-2 border-gray-200'>
